...
C
	...
	DO 1 I=1,3
   	CALL RFERMI(R(I),AR(I),2,R2(I),DEFSR)
   1	SR(I)=TEMP*R2(I)
C
...
! 	WRITE(*,*)'BELLIB gives EPSILON=',EPSILON
	TEMP=3.*HBARC2*Z2A/(16.*MC2*TWOPI)
	DO 2 I=1,3
   	CALL RFERMI(R(I),AR(I),2,R2(I),DEFSR)
   	CALL RFERMI(R(I),AR(I),4,R4(I),DEFSR)
...
C
	...
	DO 3 I=1,3
   	CALL RFERMI(R(I),AR(I),LN,RLN(I),DEFSR)
   3	SR(I)=TEMP*RLN(I)
	ENDIF
	RETURN
	END

	SUBROUTINE RFERMI(RR,AR,N,RN,DEFSR)
	IMPLICIT REAL*8(A-H,O-Z)
C
